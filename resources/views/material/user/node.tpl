																...
																{foreach $nodes as $node}
																	...
																	<div class="card">
																		<div class="card-main">
																			...
																			{else}
																				<p>IP地址：<span class="label label-brand-accent"> 
																				{$node->server}
																			{/if}
																			...
																			{if $node->sort == 0||$node->sort==7||$node->sort==8}
																				...
																				{if $node->sort==0&&$node->custom_rss==1&&$config['enable_rss']=='true'}
																					<p>连接端口：<span class="label label-brand-port"> 
																						{$user->port}
																					</span></p>
																					
																					<p>连接密码：<span class="label label-brand-pass"> 
																						{$user->passwd}
																					</span></p>
																					
																					<p>协议：<span class="label label-brand-xieyi"> 
																						{$user->protocol}
																					</span></p>
																				...
																				<span class="label label-zhushi">&nbsp;&nbsp;注:比如您消耗了1MB的流量，实际会按照{$node->traffic_rate}MB扣取！</span>
																				</p>
